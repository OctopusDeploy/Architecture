...
- **API Inputs**: The inputs used by Octopus Server and passed to the API layer
- **Execution Inputs**: The inputs used by the step executor

# Scenarios
## Required values

Values should be required by default.
...
the `foo` value should be required for all bounded contexts, since it should be invalid to pass an empty value like `undefined` at any of these points in time. The Schema Validation should assert that any step inputs provided through the API also conform to this requirement.
...
If you want this type to be required at execution time, you might implement a validation rule to validate that the type is not `undefined`. At this point, the validation rule would also execute when you try to configure this value through the API.

**There are any useful cases where this applies, so we _don't_ need to support this case.**
...
However, when configuring the UI, the value can still be undefined. To see why this is a valid case, consider the following cases:
...
Cons
- This is highly ambiguous. If you omit this `Required` mapped type and just left it as `number`, what would this mean? I would guess this means it is optional because it is not required. Which of the two flavours of optional types does this represent (optional at execution time, or optional at UI configuration time)? Is there a way to distinguish between those two options? How does this differ from `number | undefined` or `foo?: number`?
...
Cons
- How do you distinguish between the two flavours of optional types (optional at executoin time, or optional at UI configuration time)? One option might be two mapped types, like `OptionalDuringExecution<T>` and `OptionalDuringConfiguration<T>`.
...
This feels like a non-starter because of the additional friction involved in authoring steps, and other infrastructural pieces that might be needed (eg asserting at build time that the two types are compatible with each other).
...
We think this option is also a non-starter because of the massive increase in complexity (particularly when it comes to dealing with union types), the extra difficulty with implementing parsers, the lack of explicitness about what the execution inputs will look like.

## Preferred solution

For cases where the input value is required, we will rely on typescript to assert a value is provided.
...
For cases where the input value can be optional at execution time, we will make the type optional too:
...
We therefore introduce a special type (and corresponding special value) called `EmptyInitialValue` (naming TBC).
